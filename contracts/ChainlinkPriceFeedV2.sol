...
import { IChainlinkPriceFeed } from "@perp/perp-oracle-contract/contracts/interface/IChainlinkPriceFeed.sol";
...
contract ChainlinkPriceFeedV2 is IChainlinkPriceFeed, IPriceFeedV2, BlockContext, CachedTwap {
    ...
    constructor(AggregatorV3Interface aggregator, uint80 cacheTwapInterval) CachedTwap(cacheTwapInterval) {
        ...
        uint256 size;
        address aggregatorAddress = address(aggregator);
        // solhint-disable-next-line no-inline-assembly
        assembly {
            size := extcodesize(aggregatorAddress)
        }
        require(size > 0, "CPF_ANC");
